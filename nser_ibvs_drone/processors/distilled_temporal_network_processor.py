...
class DistilledTemporalNetworkProcessor(IBVSYoloProcessor):
    """Basic video processor that only displays frames from the video stream."""

    def __init__(
            self,
            model_path: str | Path = Paths.SIM_STUDENT_NET_TEMPORAL_PATH,
            logs_parquet_path: str | Path | None = Paths.LOG_PARQUET_DIR,
            error_window_size: int = 5,
            **kwargs
    ):
        super().__init__(**kwargs)
        self.student_engine = TemporalStudentEngine(model_path)
        self.min_frames_for_buffer = 3
    ...
    def _process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        detector_frame = np.array(frame)
        results = self.detector.detect(detector_frame)
        target_data = self.detector.find_best_target(detector_frame, results)
