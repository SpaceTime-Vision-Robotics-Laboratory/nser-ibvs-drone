...
def analyze_flight_durations(durations_df):
    ...
    print(f"Successful flights: {sum(durations_df['status'] == 'complete')}")
    print(f"Success rate: {sum(durations_df['status'] == 'complete') / len(durations_df) * 100:.1f}%")
    ...
    success_by_direction = durations_df.groupby('direction').agg({
        'status': lambda x: sum(x == 'complete') / len(x) * 100,
    ...
    # Success rate by direction
    success_rates = durations_df.groupby('direction')['status'].apply(lambda x: sum(x == 'complete') / len(x) * 100)
...
def create_summary_report(logs_df, durations_df):
    ...
    if not durations_df.empty:
        ...
        successful_flights = sum(durations_df['status'] == 'complete')
