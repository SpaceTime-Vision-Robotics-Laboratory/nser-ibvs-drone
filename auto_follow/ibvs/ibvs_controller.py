...
class ImageBasedVisualServo:
    def __init__(
        ...
        self.lambda_factor = np.diag([lambda_factor * 1.5, lambda_factor * 1.5, lambda_factor])
    ...
    def compute_velocities(self, verbose: bool = False) -> np.ndarray:
        ...
        if self.err_uv_values[-1] < self.err_threshold:
            self.lambda_factor = np.diag([0.5, 0.5, 0.1])
        else:
            self.lambda_factor = np.diag([self.lambda_factor_val * 2.5, self.lambda_factor_val * 2.5, self.lambda_factor_val * 1.15])
