...
class DroneCommandDataset(Dataset):
    ...
    def _collect_samples(self, scenes: list[str]) -> None:
        for scene in scenes:
                ...
                if run.is_dir():
                    ...
                    for label_path, image_path in zip(run.glob("labels/*.txt"), run.glob("images/*.jpg")):
                        if label_path.stem != image_path.stem:
                            print(
                                f"Label file {label_path} does not match image file {image_path} for scene {scene} run {run}")
                            continue
        ...
        try:
            with open(label_path, 'r') as f:
                ...
                if line:
                    ...
                    if len(values) >= self.num_commands:
